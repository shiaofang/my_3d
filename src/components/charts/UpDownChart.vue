...
const LABELS = ['上上上', '上上下', '上下上', '上下下', '下上上', '下上下', '下下上', '下下下']
const BASE_COLORS = [
...
const filteredStats = computed(() => {
  ...
  for (const draw of props.draws) {
    ...
    const idx = draw.digits.reduce((acc, digit) => acc * 2 + (digit >= 5 ? 1 : 0), 0)
    counts[idx]++
  }
...
/** Omission per pattern from full loaded history (independent of the window selector). */
const omissions = computed(() => {
  ...
  for (let i = 0; i < 8; i++) {
    ...
    for (let j = props.draws.length - 1; j >= 0; j--) {
      const idx = props.draws[j].digits.reduce((acc, d) => acc * 2 + (d >= 5 ? 1 : 0), 0)
      if (idx === i) break
      gap++
    }
    gaps[i] = gap
